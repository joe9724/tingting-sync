...
func GetDashboardData() {
	go func() {
		...
		db.Raw("select count(*) from members where to_days(ts) = to_days(now())").Find(&number_newuser)
        fmt.Println("number_newuser is",number_newuser)

         //今日购买专辑
         var number_today_buy_albums int64
         db.Raw("select count(*) from orders  where to_days(ts) = to_days(now())").Find(&number_today_buy_albums)
		 fmt.Println("number_today_buy_albums is",number_today_buy_albums)
         //今日成交额
         var money_today int64
         db.Raw("select sum(value) from orders where to_days(ts) = to_days(now())").Find(&money_today)
		 fmt.Println("money_today is",money_today)
         //今日上传录音
         var number_today_record int64
         db.Raw("select count(*) from records").Find(&number_today_record)
		 fmt.Println("number_today_record is",number_today_record)
         //月内购买专辑数
         var number_month_buy_albums int64
         db.Raw("select count(*) from orders where status=0 and DATE_SUB(CURDATE(), INTERVAL 30 DAY) <= date(createtime)").Find(&number_month_buy_albums)
		 fmt.Println("number_month_buy_albums is",number_month_buy_albums)
         //月内成交额
         var money_month_buy_albums int64
         db.Raw("select sum(value) from orders where status=0 and DATE_SUB(CURDATE(), INTERVAL 30 DAY) <= date(createtime)").Find(&money_month_buy_albums)
         ...
         db.Raw("select count(*) from sub_category_items where to_days(time) = to_days(now())").Find(&number_today_add_category)
		 fmt.Println("number_today_add_category is",number_today_add_category)
         //今日新增专辑数
         var number_today_add_albums int64
         db.Raw("select count(*) from albums where to_days(date_format(from_UNIXTIME(`time`),'%Y-%m-%d')) = to_days(now())").Find(&number_today_add_albums)
		 fmt.Println("number_today_add_albums is",number_today_add_albums)
         //今日新增书本数
         var number_today_add_book int64
         db.Raw("select count(*) from books where to_days(date_format(from_UNIXTIME(`time`),'%Y-%m-%d')) = to_days(now())").Find(&number_today_add_book)
		 fmt.Println("number_today_add_book is",number_today_add_book)
         //今日新增章节数
         var number_today_add_chapter int64
         db.Raw("select count(*) from chapters where to_days(date_format(from_UNIXTIME(`time`),'%Y-%m-%d')) = to_days(now())").Find(&number_today_add_chapter)
